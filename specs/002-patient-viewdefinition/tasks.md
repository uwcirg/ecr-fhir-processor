...
- [X] T025 [US1] (Added 2026-06-15, FR-013/R7) Add a top-level `where` to `viewdefinitions/patient.ViewDefinition.json` scoping the view to Patients persisted by this project's processor — match the feature-001 provenance tag (`meta.tag` system `…/processed-by`, code `ecr-fhir-processor`), version-agnostic — so unrelated Patients on the same server are excluded. Covered by `test_provenance_where_filter_present`. **Operator must re-run `publish_views.py` to re-materialize, then re-verify SC-001 against the project-tagged Patient count.**
...
- [~] T024 Run the full `quickstart.md` end-to-end validation against an Aidbox ≥ 2508 holding the fixture Patients: dry-run (§1), publish+materialize (§2), query `sof.patient_view` for N rows + correct columns (§3, SC-001/SC-002/SC-005), idempotent re-run (§4, SC-003), induced-failure surfacing (§5, SC-004), and empty-set edge case (§6) — **§1 / §2 / §4 CONFIRMED LIVE by operator (2026-06-15)** against a real Aidbox: dry-run plans the calls; publish OK `HTTP 201` (create) + materialize OK → `sof.patient_view (view)`; idempotent re-run yields the same outcome with publish `HTTP 200` (update-in-place, no duplicate) — SC-003 proven. **§3 row-count CONFIRMED (2026-06-15): SC-001 proven** — `count(*) FROM sof.patient_view` = 17 = `count(*) FROM patient`; `count(*)` = `count(DISTINCT id)` = 17 (one row per first-class Patient, no duplicates, no drops). **Remaining: §3 column population** (SC-002/SC-005 — columns reflect source, absent → NULL, never fabricated; pending a sample-row inspection), **§5** (induced non-conformant view → server reason surfaced, non-zero exit, SC-004), **§6** (empty Patient set → success, exit 0).
